...
# --- author
tracked(d, 300, "О. О. РОЗЕНБЕРГ", font(reg, 60), INK, track=13)

d.line([(W // 2 - 190, 410), (W // 2 + 190, 410)], fill=CINNABAR, width=3)

# --- title
ty = 560
for line, size in (("О МИРОСОЗЕРЦАНИИ", 96), ("СОВРЕМЕННОГО", 96),
                   ("БУДДИЗМА", 96), ("НА ДАЛЬНЕМ ВОСТОКЕ", 96)):
    tracked(d, ty, line, font(bold, size), INK, track=5)
    ty += 138
...
# --- imprint block
iy = oy + 190
tracked(d, iy, "Лекция, читанная на Первой", font(ital, 50), MUTED)
tracked(d, iy + 74, "буддийской выставке в Петербурге", font(ital, 50), MUTED)

# --- foot
d.line([(W // 2 - 120, H - 560), (W // 2 + 120, H - 560)], fill=INK, width=1)
tracked(d, H - 500, "ПЕТРОГРАД", font(reg, 58), INK, track=16)
tracked(d, H - 400, "1919", font(reg, 58), CINNABAR, track=16)

tracked(d, H - 250, "Отдел по делам Музеев", font(ital, 40), MUTED)
